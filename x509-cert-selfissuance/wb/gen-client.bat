...
::
set subject-display-name=deploy-service-component
::
set cert-directory=client-%subject-display-name%
set cert-file-base-name=client-%subject-display-name%
::
echo ====================
...
openssl ca -config openssl.cnf -in %cert-directory%/%cert-file-base-name%-req.pem -out %cert-directory%/%cert-file-base-name%-cert.cer -notext -extensions client_extensions
...
openssl x509 -fingerprint -in %cert-directory%/%cert-file-base-name%-cert.cer -out %cert-directory%/%cert-file-base-name%-fingerprint.txt
::
echo ==============================================================
echo Done.
::
